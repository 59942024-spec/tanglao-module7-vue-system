...
onMounted(() => {
  ...
  addNotification('🔓 Admin Dashboard opened')
    if (localStorage.getItem('loggedIn') === 'true') isLoggedIn.value = true
})
...
const doLogin = () => {
  ...
  if (!loginUsername.value.trim()) {
    loginError.value = '⚠️ Enter Username!'
    return
  }
  if (!loginPassword.value.trim()) {
    loginError.value = '⚠️ Enter Password!'
    return
  }
  if (loginUsername.value === ADMIN_USER && loginPassword.value === ADMIN_PASS) {
    isLoggedIn.value = true
    localStorage.setItem('loggedIn', 'true')
    loginUsername.value = ''
    loginPassword.value = ''
    showFeedback('success', '✅ Welcome! Login successful.')
    addNotification('🔓 Admin logged in')
  } else {
...
const doLogout = () => {
  // 1. Set logged status to FALSE → HIDES admin, SHOWS login screen
  isLoggedIn.value = false
  
  // 2. Clear saved login
  localStorage.removeItem('loggedIn')
  
  // 3. Go back to homepage (clean state)
  currentView.value = 'homepage'
  
  // 4. Feedback
  showFeedback('info', '👋 Logged out successfully.')
  addNotification('🔒 Admin logged out')
}
...
<!-- 🔐 LOGIN SCREEN -->
<div v-if="!isLoggedIn" class="flex items-center justify-center px-4 py-8 min-h-[calc(100vh-180px)]">
  <div :class="['w-full max-w-md rounded-3xl shadow-2xl p-8 sm:p-10 border-2', isDarkMode ? 'bg-gray-800 border-red-800' : 'bg-white border-red-200']">
    <h2 class="text-2xl sm:text-3xl font-bold text-center mb-8 text-red-700">🔐 Admin Login</h2>
    <div class="space-y-6">
      <div>
        <label class="font-bold block mb-2 text-lg">👤 Username</label>
        <input v-model="loginUsername" type="text"
          :class="['w-full p-4 text-lg border-2 rounded-xl', isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-red-200']"
          placeholder="Enter username" @keyup.enter="doLogin()" />
      </div>
      <div>
        <label class="font-bold block mb-2 text-lg">🔒 Password</label>
        <input v-model="loginPassword" type="password"
          :class="['w-full p-4 text-lg border-2 rounded-xl', isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-red-200']"
          placeholder="Enter password" @keyup.enter="doLogin()" />
      </div>
      <p v-if="loginError" class="text-red-500 font-bold text-center text-lg">{{ loginError }}</p>
      <button @click="doLogin"
        class="w-full py-4 bg-red-600 text-white rounded-xl font-bold text-xl hover:bg-red-700 transition-all mt-2">
        ✅ Login
      </button>
      <p class="text-base text-center mt-6 opacity-70">Please Enter the Right Username and Admin Password!</p>
    </div>
  </div>
</div>

<!-- ✅ ALL MAIN CONTENT — ONLY SHOWS WHEN LOGGED IN -->
<div v-if="isLoggedIn">
    <!-- FEEDBACK -->
...
<div class="relative">
  ...
  <button @click="isMenuOpen = !isMenuOpen"
    class="px-4 py-2 text-xl font-bold rounded-xl bg-red-100 text-red-700 hover:bg-red-200 transition-all">
    ☰ Menu
  </button>

  <!-- Dropdown Menu -->
  <div v-if="isMenuOpen"
    class="absolute left-0 mt-2 w-60 bg-white rounded-2xl shadow-2xl border-2 border-red-200 p-3 space-y-2 z-50">
    
    <button @click="currentView='register-customer'; isMenuOpen=false"
      class="w-full text-left px-4 py-3 rounded-xl bg-blue-100 hover:bg-blue-200 transition-all">
      📝 Register Customer
    </button>

    <button @click="currentView='homepage'; isMenuOpen=false"
  class="w-full text-left px-4 py-3 rounded-xl bg-yellow-100 hover:bg-yellow-200 transition-all">
  📊 Dashboard
</button>
<button @click="currentView='add'; isMenuOpen=false"
  class="w-full text-left px-4 py-3 rounded-xl bg-orange-100 hover:bg-orange-200 transition-all">
  ➕ Add Menu
</button>
<button @click="currentView='list'; isMenuOpen=false"
  class="w-full text-left px-4 py-3 rounded-xl bg-sky-100 hover:bg-sky-200 transition-all">
  📋 Menu List (6)
</button>

    <button @click="currentView='safety'; isMenuOpen=false"
      class="w-full text-left px-4 py-3 rounded-xl bg-teal-100 hover:bg-teal-200 transition-all">
      🛡️ Safety
    </button>

    <button @click="currentView='page'; isMenuOpen=false"
      class="w-full text-left px-4 py-3 rounded-xl bg-pink-100 hover:bg-pink-200 transition-all">
      📄 Page
    </button>

    <hr class="border-red-200 my-2" />

    <button @click="doLogout(); isMenuOpen=false"
  class="w-full text-left px-4 py-3 rounded-xl bg-red-100 hover:bg-red-200 text-red-700 font-bold transition-all">
  🚪 Logout
</button>
  </div>
</div>
          ...
          <div v-if="customerRecords.length===0" class="py-10 text-center rounded-xl" :class="isDarkMode ? 'bg-gray-800/50' : 'bg-white'">
           ...
           <div v-for="cust in filteredCustomerRecords" :key="cust.id" :class="['p-4 rounded-xl border-2', isDarkMode ? 'bg-gray-800 border-blue-900/30' : 'bg-white border-blue-100']">
              ...
              <div v-else>
                ...
                <p class="text-sm te
                xt-emerald-600 font-bold">💱 Change: ₱{{ cust.change || '0.00' }}</p>
